...
class SQLite:
    ...
    def clear_bd(self, model):
        ...
        set_func(function_name, self.tag, status)
...
class PersonSQLite(SQLite):
    ...
    # TODO добавить лог нового человека
    def set_person(self, model):
        function_name = "set_person"
        set_func(function_name, self.tag, status)

        args = [str(model.id), model.first_name, model.middle_name,
                model.last_name, model.specialization, model.created]
        ...
        VALUES (?, ?, ?, ?, ?, ?);
       """
        set_inside_func(f"Добавлен {args[3]}, {args[1]}, {args[2]}", function_name, tag)
...
class QuestionnairesSQLite(SQLite):
    ...
    # TODO добавить лог новой анкеты
    ...
    # TODO добавить лог удаления из бд (DEBUG)
